...
public class ExcelReader {
	...
	public void renameSheet(int indexOfSheet, String name) throws IOException {
		fis = new FileInputStream(path); 
		workbook = new XSSFWorkbook(fis); 
		workbook.setSheetName(indexOfSheet, name);
	...
	public boolean setCellData(String sheetName,String colName,int rowNum, String string){
		try{
		fis = new FileInputStream(path); 
		workbook = new XSSFWorkbook(fis);
...
public List<Integer>  getCellsRowNums(String sheetName,String colName,String cellValue){
		List<Integer> rownumList = new ArrayList<Integer>();
		for(int i=2;i<=getRowCount(sheetName);i++){
			//System.out.println(getCellData(sheetName,colName , i));
	    	if(getCellData(sheetName,colName , i).replace(".0", "").contains(cellValue)){
	    	//	System.out.println(getCellData(sheetName,colName , i).replace(".0", ""));
	    		rownumList.add(i);
